'''
Script to execute functions from classical_descriptors.py and moments_descriptors.py in order to generate all descriptor data for the given structures in GASdb datasets. Descriptors include
'''
import pickle
import numpy as np
import pandas
...
init_or_final_str = 'init'  # Set to init or fin for unrelaxed or relaxed
mat_list_file_loc = './predictions_non_metals_h.pkl'    # Set name of file
...
print(df['ae'].mean())
print(df['ae'].std())
with open('/path/to/pickle/file/predictions_non_metals_h_full.pkl','wb') as pf:

...
from train_sr import train_dgl
from config import TrainingConfig
import ignite
import ignite.distributed as idist
from train_sr import group_decay, setup_optimizer
###
...
def train_for_folder(
    ...
    #)
    t1 = time.time()
    print('train_for_folder_3\n\n\n')
    ### shafigh
    argdict={'config' : config, 'dataset_array' : dataset}
    if config.distributed:
        spawn_kwargs={'nproc_per_node': config.num_workers, 'master_addr' : "127.0.0.1", 'master_port' : "8888", 'nnodes' : 1}
        backend = "nccl"
        #train_val_test_loaders=[train_loader, val_loader, test_loader, prepare_batch, ]
        #argdict={'config' : config, 'train_val_test_loaders' : train_val_test_loaders}    
        with idist.Parallel(backend=backend, **spawn_kwargs) as parallel:
        #parallel.run(train_dgl, args=(config,train_val_test_loaders,))
        #parallel.run(train_dgl, config, train_val_test_loaders)
        #parallel.run(train_dgl, config)
        #parallel.run(train_dgl, config, train_val_test_loaders=[ train_loader, val_loader, test_loader, prepare_batch, ],)
            parallel.run(train_dgl, argdict)
    else:
        h, mae_test = train_dgl(0, argdict)
    ### shafigh

    # shaym/
    #mp.set_sharing_strategy('file_system')
    #mp.spawn(train_dgl, nprocs=2, args=(config,train_val_test_loaders))
    #/shaym

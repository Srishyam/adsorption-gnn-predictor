...
def train_dgl(local_rank, argdict):
#    print('local_rank: %s\n\n\n' % local_rank)
    ...
    if torch.cuda.is_available():
        device=idist.device()
    print('train_dgl_0 device')
    print(device)
    print('\n\n\n')
    # /Shyam
#    train_val_test_loaders = argdict["train_val_test_loaders"]
    (
        #dataset_train,
        train_loader,
        val_loader,
        ...
        dataset_array= argdict["dataset_array"],
    ...
    ### overwrite the train_loader and test_loader
    if idist.get_local_rank() > 0 and config.distributed:
        # Ensure that only local rank 0 download the dataset
        idist.barrier()

    # Setup data loader also adapted to distributed config: nccl, gloo, xla-tpu
    #print('trainpy_11 ',len(dataset_train))
    #print('\n\n\n')
    #print('train_loader length = ', len(train_loader.dataset))
    #train_loader = idist.auto_dataloader(
    #   train_loader.dataset, batch_size=8, num_workers=2, shuffle=True, drop_last=True
       #dataset_train, batch_size=8, num_workers=2, shuffle=True, drop_last=True
       #dataset_train, batch_size=config["batch_size"], num_workers=config["num_workers"], shuffle=True, drop_last=True
       #[1,2,3], batch_size=config["batch_size"], num_workers=config["num_workers"], shuffle=True, drop_last=True
    #)
    #print('trainpy_0\n\n\n')
    #print('train_loader length = ', len(train_loader.dataset))
...
#            # Shyam/
#            print('shyam_setup_2')
#            print('rank = ' + str(rank) + ' world size = ' + str(world_size) + '\n\n\n')
#            # /Shyam
#            # Shyam/
#            mp.spawn(train, nprocs=2)
#            # /Shyam
...
#        local_rank = int(os.environ["LOCAL_RANK"])
        ...
        #port = find_free_port()
        #port ="12355"
        #mp.spawn(setup, args=[world_size,port], nprocs=world_size)
        # /Shyam
        # /Shyam
        print('trainpy_1\n\n\n')
        #print('local rank=%\n\n\n'%local_rank)

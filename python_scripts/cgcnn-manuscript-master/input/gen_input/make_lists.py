...
#def make_SDT_list(docs_loc, dir_loc, start=1, end=100):
def make_SDT_list(docs_loc, dir_loc, procs=110):
    ...
    with mp.Pool(procs) as pool:
        SDT_list_object = list(tqdm.tqdm(pool.imap(lambda x: SDT_out[x], range(len(SDT_out)), chunksize=40), total=len(SDT_out)))
    with open('%s/SDT_list.pkl'%dir_loc, 'wb') as SDT_file:
        pickle.dump(SDT_list_object, SDT_file)

#def make_target_list(docs_loc, dir_loc, start=1, end=100):
def make_target_list(docs_loc, dir_loc):
    ...
    for data in data_dict:
        data['surface'] = get_surface_from_doc(data)

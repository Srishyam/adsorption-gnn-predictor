...
homo_list = json.load(open('./HOMO_list.json', 'r'))
homo_indices = [i for i in range(1,len(homo_list)+1)]
dataframe = pd.DataFrame(homo_list, index=homo_indices, columns=['HOMO energy'])
no_of_categs = 10
#temp, bins = pd.cut(dataframe['HOMO energy'], no_of_categs, retbins = True)
temp, bins = pd.qcut(dataframe['HOMO energy'], no_of_categs, retbins = True)
print(dataframe)

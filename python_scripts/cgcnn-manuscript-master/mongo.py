...
def Shyam_make_doc_from_atoms(atoms, **kwargs):
    '''#
...
#    atoms_dict = OrderedDict(_make_atoms_dict(atoms))
    atoms[0].tag = 1
    atoms[1].tag = 1
    atoms_dict = OrderedDict(_Shyam_make_atoms_dict(atoms))
...
def _Shyam_make_atoms_dict(atoms):
    '''
    ...
    atoms_dict = OrderedDict(#atoms=[{'symbol': atom.symbol,
                             ...
                             atoms=[{'symbol': atom.symbol,
                                     'position': json.loads(encode(atom.position.tolist())),
                                     'tag': atom.tag,
                                     'index': atom.index,
                                     'charge': atom.charge,
                                     'momentum': json.loads(encode(atom.momentum.tolist())),
                                     'magmom': atom.magmom}
                                    for atom in atoms],
...
#                  info=doc['atoms']['info'],
                  ...
                  constraint=[dict2constraint(constraint_dict)
                              for constraint_dict in doc['constraints']])
#                              for constraint_dict in doc['atoms']['constraints']])
        ...
        for key in item.keys():
            ...
            if key == 'atoms':
                ...
                for at in a:
                    ...
                    mm  = at['magmom'],
                    chg = at['charge']
                    at_arr.append(Atom(sym, pos, ta, mom, mm, chg))
    cell=doc['atoms']['cell']
    pbc=doc['atoms']['pbc']
    info=doc['atoms']['info']
    constraint = []
    for constraint_dict in doc['atoms']['constraints']:
        constraint.append([dict2constraint(constraint_dict)])
    atoms = Atoms(at_arr, cell, pbc, info, constraint)
    '''
    # /end Shyam
    ''' 
    results = doc['results']
    # Shyam start/
    calc = SinglePointCalculator(energy='%TBD%',
                                 forces='%TBD%',
                                 stress='%TBD%',
                                 atoms=atoms)
    # /end Shyam
    ...
    return atoms
# Shyam start/
def make_atoms_from_doc_ocp(doc):
    atoms = Atoms([Atom(atom.symbol, atom.position,
                        tag=atom.tag, momentum=atom.momentum,
                        magmom=atom.magmom, charge=atom.charge)
                   for atom in doc],
                  cell=doc.cell,
                  pbc=doc.pbc,
                  info=doc.info,
                  constraint=None
#                  constraint=[dict2constraint(constraint_dict) for constraint_dict in doc.constraints]
                  )

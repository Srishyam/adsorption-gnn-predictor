...
class NeuralNet:
    ...
    def notify(self, method_name, **cb_kwargs):
        ...
        * on_batch_end
        ...
        """
        # Shyam start/
        #getattr(self, method_name, 2)(self, 2, **cb_kwargs)
        # /end Shyam
        getattr(self, method_name)(self, **cb_kwargs)
        for _, cb in self.callbacks_:
            getattr(cb, method_name)(self, **cb_kwargs)
        # Shyam start/
        #print('(skorch.net.py) within notify')
        #print(self.callbacks_[5][1].lr_scheduler_._last_lr)
        # / end Shyam
    ...
    def run_single_epoch(self, dataset, training, prefix, step_fn, **fit_params):
        ...
        **fit_params : dict
        ...
        for data in self.get_iterator(dataset, training=training):
            ...
            # Shyam start/
            XXi = Xi
            XXi['crystal_atom_idx'] = torch.cat(Xi['crystal_atom_idx'], dim=0)
            self.history.record_batch(prefix + "_batch_size", get_len(XXi))
            # /end Shyam
